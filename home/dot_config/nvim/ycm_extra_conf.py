...
def GetProgramInclude(filename):
    global flags
    local_flags = []
    if 'src' in filename:
        # Try to find project headers folder
        srcdir = re.search('.*src[^/]*',filename).group()
        headers = glob.glob(os.path.join(srcdir, 'include*/'))
        headers += glob.glob(os.path.join(srcdir, '../include*/'))
        for e in headers:
